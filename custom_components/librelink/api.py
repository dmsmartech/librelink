...
class LibreLinkApiClient:
    ...
    Attributes:
        ...
        account_id: The user account ID from login, used for Account-Id header.
        base_url: For API calls depending on your location
        Session: aiottp object for the open session
    """

    def __init__(
        self, token: str, account_id: str, base_url: str, session: aiohttp.ClientSession
    ) -> None:
        """Sample API Client."""
        self._token = token
    ...
    async def async_get_data(self) -> any:
        ...
        _LOGGER.debug("async_get_data called, url: %s", self.connection_url)
        ...
        _LOGGER.debug(
            "Connections API status: %s",
            APIreponse.get("status"),
        )
...
class LibreLinkApiLogin:
    ...
    async def async_get_token(self) -> any:
        ...
        if reponseLogin["status"] == 4:
            raise LibreLinkApiAuthenticationError(
                "Account redirect: check that the region is correct or accept Terms of Service in the LibreLink Up app",
            )
        if reponseLogin["status"] != 0:
            raise LibreLinkApiError(
                f"Unexpected API status: {reponseLogin['status']}",
            )

        monToken = reponseLogin["data"]["authTicket"]["token"]
        monAccountId = reponseLogin["data"]["user"]["id"]
        _LOGGER.debug("Login successful, account_id: %s", monAccountId)

        return monToken, monAccountId
...
async def api_wrapper(
    ...
    if response.status in (401, 403):
        ...
        _LOGGER.error("HTTP %s response body from %s: %s", response.status, url, body)
        raise LibreLinkApiAuthenticationError("Invalid credentials")

    if response.status == 430:
        body = await response.text()
        _LOGGER.error("HTTP 430 response body: %s", body)
        raise LibreLinkApiAuthenticationError(
            "Terms of Service not accepted: open the LibreLink Up app and accept the Terms of Service",
        )

    try:
        _LOGGER.debug("HTTP response status %s from %s", response.status, url)
        if not response.ok:
            body = await response.text()
            _LOGGER.error("HTTP %s response body from %s: %s", response.status, url, body)
